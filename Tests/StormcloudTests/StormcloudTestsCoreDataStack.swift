...
open class CoreDataStack {
	...
	*/
	...
	func saveToBinary() {
		...
		do {
			try self.persistentStoreCoordinator?.migratePersistentStore(
				self.persistentStoreCoordinator!.persistentStores.first!,
				to: FileManager.default.temporaryDirectory.appendingPathComponent("BinaryStore"),
				options: nil,
				withType: NSBinaryStoreType)
